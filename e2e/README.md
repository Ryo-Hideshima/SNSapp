...
実際のブラウザで「登録→ログイン→投稿→いいね→フォロー」等の一連のユーザー操作を検証するE2Eテスト一式。`.github/workflows/ci.yml`の`e2e`ジョブでPR・main push時に**chromiumプロジェクトのみ**自動実行される。Firefox/WebKitスモークはCIには含めておらず、ローカルで任意実行する運用(下記「クロスブラウザ方針」を参照)。`performance-tests/`(k6)は今回のCI化の対象外。
